...
pub fn solve() {
    ...
    for row in 0..TRIANGLE.len() {
        for col in 0..(TRIANGLE[row].len()-row) {
            if memoized_row[col] > memoized_row[col + 1] {
                memoized_row[col] = memoized_row[col] + TRIANGLE[row][col];
            } else {
                memoized_row[col] = memoized_row[col + 1] + TRIANGLE[row][col];
            }
        }
    }
    println!("The largest summing path through the triangle sums to {}", memoized_row[0]);
}